<template>
  ...
  >
      ...
      >
        <div v-if="!isAnnouncement(message)">
          <span class="message-header">
              ...
              v-if="
                index === 0 ||
                  message.author.username !==
                    sortedMessages[index - 1].author.username
              "
              class="author"
            ...
            >
              ...
              <span class="author-info">
                <span
                  v-if="message.author.name"
                  class="author-info-item username"
                >
                  ({{ message.author.username }})
                </span>
                <a
                  :href="
                    `https://www.codewars.com/users/${message.author.username}/completed`
                  "
                  target="_blank"
                  >View on Codewars</a
                >
          ...
          <div class="message-content-ctn">
            <div
              class="sent-at"
              v-html="formatMessageDate(message.created_at)"
            />
    ...
    <template v-slot:controls>
      <div
        id="msg-input"
        :class="['with-prompt', { multiline: multilineInput, code: codeInput }]"
        ref="input"
      >
        <textarea
          id="msg-textarea"
        ...
        ></textarea>
        <a
          v-if="settings.room.voice_chat_url"
          :href="settings.room.voice_chat_url"
          target="_blank"
        >
          <std-button
            small
            ...
            :class="['join-call animated flipInX', { flipOutX: input }]"
          />
        </a>
      </div>
    </template>
  ...
  methods: {
    getUserOS() {
      if (navigator.appVersion.indexOf("Win") != -1)
        this.submitHint = "[Ctrl]+[Enter]";
      if (navigator.appVersion.indexOf("Mac") != -1)
        this.submitHint = "[⌘]+[Enter]";
    ...
    updateTextAreaRows() {
      ...
      const extraRows = Math.ceil(
        (textarea.scrollHeight - this.baseScrollHeight) / this.inputRowHeight
      );
      textarea.rows = Math.min(minRows + extraRows, this.inputMaxRows);
      if (textarea.rows !== previousRows && atBottom)
        this.autoScrollToLastMessage();
    },
    getInputRowHeight() {
      const textarea = this.$refs.textarea;
    ...
    replaceUsername(str) {
      ...
      handles.forEach(handle => {
        ...
        const userIndex = this.authors.findIndex(
          author => author.username === username
        );
        const author =
          userIndex >= 0
            ? `<span class="highlight" title="${username}">${this.authors[
                userIndex
              ].name || username}</span>`
            : username;
    ...
    displayMsg(msg) {
      return `${this.replaceUsername(
        msg.content
      )}<div class="sent-at">${this.formatMessageDate(msg.created_at)}</div>`;
    },
    ...
    messagesScrolledToBottom() {
      ...
      return (
        messages.scrollTop >= messages.scrollHeight - messages.clientHeight - 50
      );
    },
    ...
    addTabCharacter(e) {
      ...
      e.target.value =
        val.substring(0, start) + tabCharacter + val.substring(end);
    ...
    hasContent(message) {
      return (
        message.content.split(/^```\w*$/gm).filter(block => trim(block))
          .length > 0
      );
    }
  }
