  ...
  >
    <div class="d-flex flex-column h-100">
      <div class="fixed-header">
        <table
          :class="['console-table h-100', { 'no-stats': !room.show_stats }]"
        >
          <thead>
            <tr>
              <th
                scope="col"
                :style="room.show_stats ? 'width: 50%;' : 'width: 100%;'"
              >
                <span class="data"
                  >WARRIORS [{{ sortedLeaderboard.length }}]</span
                >
              </th>
              <th v-if="room.show_stats" scope="col" style="width: 6%;">
                <span class="data">#</span>
              </th>
              <th v-if="room.show_stats" scope="col" style="width: 12%;">
                <span class="data">SCORE</span>
              </th>
              <th v-if="room.show_stats" scope="col" style="width: 12%;">
                <span class="data">BATTLES</span>
              </th>
              <th v-if="room.show_stats" scope="col" style="width: 20%;">
                <span class="data">WON : LOST</span>
              </th>
          ...
          <tbody>
            <tr
              ...
              :class="{ 'highlight current-user': isCurrentUser(user.id) }"
              :title="
                `${
                  user.username
                } | ${-user.codewars_overall_rank} kyu | ${user.total_survived ||
                  0} wins on SpeedBattles`
              "
              :key="user.id"
            >
              <th scope="row">
                <div class="d-flex justify-content-between">
                  <span :class="['data user', { offline: !user.online }]">
                    <span :class="['mr-1', { 'online highlight': user.online }]"
                      >●</span
                    >
                  ...
                  <span class="invite-button">
                    <std-button
                      v-if="showInviteButton(user.id)"
                      @click.native="toggleInvite(user.id)"
                      :title="showInviteButton(user.id)"
                      small
                      class="mr-2"
                    />
                  </span>
              ...
              <td v-if="room.show_stats">
                <span class="data rank">{{ userRanks[index] }}</span>
              </td>
              <td v-if="room.show_stats">
                <span class="data">{{
                  user ? displayScore(user.total_score) : "-"
                }}</span>
              </td>
              <td v-if="room.show_stats">
                <span class="data">{{ user ? user.battles_fought : "-" }}</span>
              </td>
              <td v-if="room.show_stats">
                <span class="data">{{
                  user
                    ? `${user.battles_survived || "-"} : ${user.battles_lost ||
                        "-"}`
                    : "-"
                }}</span>
              </td>
...
export default {
  ...
  computed: {
    ...
    userRanks() {
      ...
      return this.sortedLeaderboard.map((user, index) => {
        ...
        if (
          index === 0 ||
          user.total_score < this.sortedLeaderboard[index - 1].total_score
        ) {
          rank = realRank;
        }
        return rank;
  ...
  methods: {
    ...
    userRank(index) {
      ...
      if (
        this.displayScore(this.sortedLeaderboard[index].total_score) ===
        this.displayScore(this.sortedLeaderboard[index - 1].total_score)
      )
        return "."; // '↑'
    ...
    toggleInvite(userId) {
      if (
        this.findUser(userId).invite_status == "eligible" &&
        this.currentUserIsModerator
      )
        this.$root.$emit("invite-user", userId);
