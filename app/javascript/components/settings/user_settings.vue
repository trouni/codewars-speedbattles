  ...
  <div>
    ...
    <div class="form-group mt-3 my-4" v-if="settings.user.signed_in">
      <div class="d-flex justify-content-between align-items-center">
        <h5 class="m-0 no-wrap">Codewars username</h5>
        <input
          disabled
          type="text"
      ...
      <small>You can't change your Codewars username.</small>
    </div>
    <div class="form-group mt-3 my-4" v-if="settings.user.signed_in">
      <div class="d-flex justify-content-between align-items-center">
    ...
    <div class="form-group my-4">
      <div class="d-flex justify-content-between align-items-center">
        ...
        <span class="d-flex justify-content-around flex-grow-1 ml-2">
          <std-button
            ...
            :fa-icon="
              `fas ${settings.user.music ? 'fa-music' : 'fa-volume-mute'}`
            "
          ...
          <std-button
            ...
            :fa-icon="
              `fas ${settings.user.voice ? 'fa-robot' : 'fa-volume-mute'}`
            "
            :disabled="!settings.room.sound && !moderator"
      ...
      <small
        >Toggle music, sound effects and voice announcements.<span
          v-if="!settings.room.sound"
          ><br /><em
            >Sound settings have been partially disabled for this war room.</em
          ></span
        ></small
      >
    </div>
    <div class="form-group my-4" v-if="settings.user.signed_in">
      <div class="d-flex justify-content-between align-items-center">
        ...
        <select v-model="hljsLang" class="flex-grow-1 ml-3">
          <option value></option>
          <option
            v-for="(key, lang_name) in settings.room.codewars_langs"
            :value="lang_name"
            :key="key"
            >{{ key }}</option
          >
      ...
      <small
        >Select a preferred language for challenges and code blocks in the
        chat.</small
      >
    </div>
    ...
    <div class="form-group my-4">
      ...
      <small
        >Alternative theme that removes transparency and other effects for
        slower devices.</small
      >
    </div>
    <div
      class="form-group my-4"
      v-if="settings.user.signed_in || settings.room.id"
    >
      <div class="d-flex justify-content-around">
        <a
          href="/users/sign_out"
          data-method="delete"
          v-if="settings.user.signed_in"
        >
          <std-button @click.native="cancel" fa-icon="fas fa-sign-out-alt" small
            >Sign out</std-button
          >
        </a>
